...
from agroecogym_engine.entities.Weather import daily_ground_irradiance
        ...
        for x in range(self.field.X):
            for y in range(self.field.Y):
                if self.variables["population#nb"][x, y].value > 0:
                    ...
                    #Compute Stress (=nutrient/water deficit)
                    total_stress = 0
                    ...
                    if self.variables["stage"][x, y].value == "entered_grow":
                        ...
                        # Sun power in kWh/m2:
                        sun_power = daily_ground_irradiance(field.localization["longitude#°"], weather.variables["day#int365"].value) * (1.-weather.variables["clouds#%"].value / 100)
                        #TODO: Check units, is sun_power in kWh.m-2.day-1?
                        ...
                        # Sun power in kWh/m2:
                        sun_power = daily_ground_irradiance(field.localization["longitude#°"], weather.variables["day#int365"].value) * (1.-weather.variables["clouds#%"].value / 100)
                        #TODO: is sun_power in kWh.m-2.day-1?
                        # sun power received by plant in kWh:
                        # sun_power *= self.parameters["grow_conditions"]["grow_leaf_surface#m2.cm-1"]* self.variables["size#cm"][(x,y)].value
