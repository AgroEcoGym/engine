...
from agroecogym_engine.utils.gymUnion import MultiUnion, Sequence
...
from  agroecogym_engine.specifications.specification_manager import (  # noqa: E402
    build_actionsyaml,
    build_inityaml,
    build_scoreyaml,
)
...
class Farm(gym.Env):
    ...
    def __init__(
        ...
        self._setup_score_configuration(farmpath)
        self._setup_initial_configuration(farmpath)
        self._setup_action_configuration(farmpath)
        self.scoring.setup(self)
        self.rules.setup(self)
        self._setup_seed(seed)
        self._setup_spaces(seed)
    ...
    def _setup_score_configuration(self,farmpath):
        if self.scoring.score_configuration is None:
            ...
            build_scoreyaml(farmpath + "_score_vanilla.yaml", self)
            self.scoring.score_configuration = (
                    farmpath + "_score_vanilla.yaml"
            )
            print(
                "[Solution]"
                + " Vanilla score configuration file automatically generated in "
                + str(farmpath + "_score_vanilla.yaml")
    ...
    def _setup_initial_configuration(self,farmpath):
        ...
        if self.rules.init_configuration is None:
            ...
            print(
                ...
                + str(farmpath + "_init_vanilla.yaml")
    ...
    def _setup_action_configuration(self,farmpath):
        if self.rules.actions_configuration is None:
            ...
            build_actionsyaml(farmpath + "_actions_vanilla.yaml", self)
            self.rules.actions_configuration = (
                farmpath + "_actions_vanilla.yaml"
            )
            print(
                "[Solution]"
                + " Vanilla action configuration file automatically generated in "
                + str(farmpath + "_actions_vanilla.yaml")
    ...
    def render_step(self, action, observation, reward, terminated, truncated, info):
        ...
        return rs(self, action, observation, reward, terminated, truncated, info)
    ...
    def understand_the_farm(self):
        ...
        for ac in actions:
            ...
            print(ac)
        print("###########GYM SPACES#################")
        from agroecogym_engine.utils.gymUnion import str_pretty
